...
filt_out <- filterAndTrim(fns, filt_fns, minQ=2, minLen=1350, maxLen=1700, maxN=0, rm.phix=FALSE, maxEE=3, multithread = FALSE) #we retain only reads with fewer than three exp. errors 
print(filt_out)
...
#106420556 total bases in 69365 reads from 12 samples will be used for learning the error rates.
...
```{r}
seqtab <- makeSequenceTable(dada_dd); dim(seqtab) #48 998
table(nchar(getSequences(seqtab))) #distibution of sequence lengths 
#shortest 1398, longest 1634
...
1-sum(seqtab.nochim)/sum(seqtab)  #0.004064076
...
# assign taxonomy using UNITE2020 reference database

```{r}

#upload DECIPHER reference database (UNITE_v2020_February2020.RData), when it is uploaded it is named as trainingSet
...
physeq
#OTU_table; 988 taxa, 48 samples
#tax_table; 988 taxa, by 7 taxonomic ranks
...
#now we remove 40 NA on kingdom level, they do not belong to Fungi

physeq0 <- subset_taxa(physeq, !is.na(kingdom) & !kingdom %in% c ("", "uncharacterized"))

table(tax_table(physeq0)[, "phylum"], exclude=NULL)
#3 ASVs belong to Ascomycota, we remove these

physeq0 <- subset_taxa(physeq0, !is.na(phylum) & !phylum %in% c ("Ascomycota"))
#we keep Glomeromycota, 945 ASVs
...
# create new phyloseq object with renamed taxonomy

```{r}

#create new physeq object with renamed taxonomy

taxa_file = read.csv("/Users/Katja/Box Sync/PhD Lund Katja/Sequencing/PacBio AMF/Sequencing data AMF/dada2/AMF_DOK trial/PacBio_AMF_workflow/results_dada2/taxonomy_glomeromycota_filt.csv", sep=",", row.names=1)
taxa_file = tax_table(as.matrix(taxa_file))

asv_file = read.csv("/Users/Katja/Box Sync/PhD Lund Katja/Sequencing/PacBio AMF/Sequencing data AMF/dada2/AMF_DOK trial/PacBio_AMF_workflow/results_dada2/ASVs_counts_filt.csv", sep=",", row.names=1)
asv_file = otu_table(as.matrix(asv_file), taxa_are_rows = FALSE)

physeq_AMF = phyloseq(asv_file, taxa_file, meta_file)

random_tree_amf = rtree(ntaxa(physeq_AMF), rooted=TRUE, tip.label=taxa_names(physeq_AMF))
plot((random_tree_amf))

#add random tree to phyloseq object 
physeq_AMF = merge_phyloseq(physeq_AMF, random_tree_amf)
physeq_AMF
...
sum(sample_sums(physeq_AMF)) #185752
sort(sample_sums(physeq_AMF), decreasing = F) 
min(sample_sums(physeq_AMF)) #376
max(sample_sums(physeq_AMF)) #11426
median(sample_sums(physeq_AMF)) #3387
mean(sample_sums(physeq_AMF)) #3869.833

#how many taxa are there? 
unique(ntaxa(physeq_AMF)) #945

```
**945 ASVs in Glomeromycota phylum**
...
#samples 85, 31, 45, 84 contain less than 1000 reads 
...
#TSS normalization
rel_ab_AMF <- transform_sample_counts(physeq_AMF_44s, function(OTU) OTU/sum(OTU))
...
#calculate envfit
set.seed(0935)
env.fit <- envfit(scores(nmds_ord), env_nmds, permutations = 999, na.rm = TRUE)
env.fit
