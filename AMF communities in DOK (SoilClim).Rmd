...
# Shannon index on genus level 

```{r}

plot_richness(physeq_AMF_genus_500, x="farming_system", measures=c("Shannon")) + theme_bw()

plot_richness(physeq_AMF_genus_500, x="treatment", color="farming_system", measures=c("Observed","Shannon")) + stat_boxplot(geom = "errorbar") + geom_boxplot() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black")) + xlab("Treatment") + ggtitle("AMF alpha diversity") + ylab("Alpha metrics")

shannon_div <- data.frame(estimate_richness(physeq_AMF_genus_500, split=TRUE, measures = c("Observed", "Shannon")), sample_data(physeq_AMF_genus_500))
shannon_div

write.csv(shannon_div, "results/shannon_div.csv")

brms_shannon <- shannon_div
head(brms_shannon)
str(brms_shannon)

shannon_brms_genus <-brm(formula = (Shannon)~time*treatment*farming_system+(1|block/plot),
            data = brms_shannon,
            family="gaussian",
            seed = 1041,
            warmup = 2000,#The number of warmup iterations should 
...
pp_check(shannon_brms_genus)

# Save an object to a file
saveRDS(shannon_brms_genus, "data/rds/shannon_brms_genus.rds")

#reload it so you don't have to run it again
shannon_brms_genus <- readRDS("data/rds/shannon_brms_genus.rds")

#check model fit 
yfit_shannon_genus <- brms_shannon$Shannon
yrep_shannon_genus <- posterior_predict(shannon_brms_genus, draws = length(yfit_shannon_genus))

launch_shinystan(shannon_brms_genus)

#explore results
summary(shannon_brms_genus)

#marginal effects
marginal_effects(shannon_brms_genus)

newdat_genus <- expand.grid(
  time = factor(c("4w", "13w"),levels=levels(brms_shannon$time)),
  farming_system = factor(c("BIODYN", "CONMIN"), levels=levels(brms_shannon$farming_system)),
  treatment = factor(c("C","R", "RC"),levels=levels(brms_shannon$treatment)))
head(newdat_genus)


fit_shannon_genus = fitted(shannon_brms_genus, newdata = newdat_genus
                           , robust=T, re_formula = NA,summary = TRUE,probs = c(0.025, 0.975))
fit_shannon_genus

# no colnames, I add them manually
fitdf_shannon_plot_genus = cbind(newdat_genus, fit_shannon_genus)
fitdf_shannon_plot_genus

#change order of treatment in fitted values
fitdf_shannon_plot_genus$treatment<-factor(fitdf_shannon_plot_genus$treatment, levels=c("R", "RC", "C"))

#prepare and extract results table
fitdf_shannon_genus<-fitdf_shannon_plot_genus[, c("farming_system","treatment", "time", "Estimate","Q2.5","Q97.5")]
write.table(fitdf_shannon_genus,"results/shannon_brms_genus.txt",sep="\t",quote=F)

#make the final plot
fitted_shannon_genus<-ggplot(data = fitdf_shannon_plot_genus, 
                     aes(x = farming_system,
                         y =Estimate,
                         color = treatment,
                         shape=treatment)) +
...
print(fitted_shannon_genus)
ggsave("results/shannon_brms_genus.tiff", width = 18, height = 10, units = "cm")

fitdf_shannon_plot_genus<-as.data.frame(fitdf_shannon_plot_genus)

#if you want, you can also calculate differences between treatments based on the posterior
##calucate differences with CrI, you need the whole posterior
#the whole posterior distribution for me. I save it as dataframe (fit1)

fit_shannon_genus = as.data.frame(fitted(shannon_brms_genus, newdata=newdat_genus,
                           re_formula = NA, summary = FALSE))

#this is the whole posterior, not summarized as before

#add names
colnames_genus<-as.character(interaction(newdat_genus$farming_system, newdat_genus$treatment, newdat_genus$time))

colnames(fit_shannon_genus)<-colnames_genus
names(fit_shannon_genus)

#calculate the mean differences between systems across the roofs at 4w
BIODYN_brms_shannon_genusT1<-(fit_shannon_genus$BIODYN.C.4w+fit_shannon_genus$BIODYN.RC.4w+fit_shannon_genus$BIODYN.R.4w)/3
mean(BIODYN_brms_shannon_genusT1)
sd(BIODYN_brms_shannon_genusT1)
CONMIN_brms_shannon_genusT1<-(fit_shannon_genus$CONMIN.C.4w+fit_shannon_genus$CONMIN.RC.4w+fit_shannon_genus$CONMIN.R.4w)/3
mean(CONMIN_brms_shannon_genusT1)
sd(CONMIN_brms_shannon_genusT1)

diff_brms_shannon_genusT1 <- (BIODYN_brms_shannon_genusT1-CONMIN_brms_shannon_genusT1)

#use the quantile function to get mean of that differences with CrI
round(quantile(diff_brms_shannon_genusT1, probs=c(0.025, 0.5, 0.975)),2)
#this is a result
mean(diff_brms_shannon_genusT1)
